...
type GPU(platformId) =     
    let context = 
        ...
        let profile = OpenCL.GetPlatformInfo(platform,CLPlatformInfo.Profile)
        let name = OpenCL.GetPlatformInfo(platform,CLPlatformInfo.Name)
        let _ = ErrorMsg.Logger.Debug <| sprintf "Selected OpenCL platform: %d name: %A profile: %A vendor: %A version: %A" platformId name profile vendor version
        ...
        if !err = CLError.Success then context' else raise <| GPUErrorException !err
