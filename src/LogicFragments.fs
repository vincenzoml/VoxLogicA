...
exception UnknownAtomicPropositionException of ap: string with
    override this.Message = sprintf "Unknown atomic proposition %s" this.ap
...
type IBooleanModel<'Value> =
    ...
    [<OperatorAttribute("bconstant",
                        "bool",
                        "valuation(bool)",
                        "The image which has the given boolean value at each voxel")>]
...
type IQuantitativeModel<'Value> =
    ...
    [<OperatorAttribute("between",
                        [| "number"; "number"; "valuation(number)" |],
                        "valuation(bool)",
    ...
    [<OperatorAttribute("abs",
                        "valuation(number)",
                        "valuation(number)",
                        "Computes an image where each voxel contains the absolute value of the corresponding voxel in the input image")>]
    ...
    [<OperatorAttribute("mask",
                        [| "valuation(number)"; "valuation(bool)" |],
                        "valuation(number)",
...
type ISpatialModel<'Value> =
    ...
    [<OperatorAttribute("cvnear", "valuation(bool)", "valuation(bool)", "Spatial-logical converse-closure")>]
    abstract member CVNear: 'Value -> Job<'Value>
...
type IDirectedSpatialModel<'Value> =
    ...
    [<OperatorAttribute("bnear",
                        "valuation(bool)",
                        "valuation(bool)",
                        "Backwards spatial-logical closure (that is, backwards dilation)")>]
    abstract member BNear: 'Value -> Job<'Value>

    [<OperatorAttribute("binterior",
                        "valuation(bool)",
                        "valuation(bool)",
                        "Backwards spatial-logical interior (that is, backwards erosion)")>]
    abstract member BInterior: 'Value -> Job<'Value>

    [<OperatorAttribute("bthrough",
                        [| "valuation(bool)"; "valuation(bool)" |],
                        "valuation(bool)",
                        "bthrough(img1,img2) is true at voxel x if there is a reverse path p, starting in x and ending in a voxel y, with y true in img1, and all points of p (including extremes) true in img2")>]
    abstract member BThrough: 'Value -> 'Value -> Job<'Value>

type IStatisticalModel<'Value> =
    inherit ILogicModel<'Value>

    [<OperatorAttribute("crossCorrelation",
                        [| "number"
                           "valuation(number)"
                           "valuation(number)"
                           "valuation(bool)"
                           ...
                           "number" |],
                        "valuation(number)",
                        "crossCorrelation(radius,local,target,mask,min,max,nbins) computes similarity scores via statistical cross-correlation (see academic papers or extended documentation)")>]
...
type IImageModel<'Value> =
    ...
    [<OperatorAttribute("percentiles",
                        [| "valuation(number)"; "valuation(bool)"; "number" |],
                        "valuation(number)",
                        "Each voxel in percentiles(img,bimg,k) is the percentile rank, between 0 and 1, of its value in img, considering only voxels that are true in bimg (voxels that are false in bimg are assigned value 0); the rank is not rounded, and it is corrected with k * n where n is the number of voxels equal to the considered one.")>]
    abstract member Percentiles: 'Value -> 'Value -> float -> Job<'Value>

    [<OperatorAttribute("rgb",
    ...
    [<OperatorAttribute("lcc",
                        [| "valuation(bool)"; "valuation(number)" |],
                        "model",
                        "Labels connected components of a boolean model. The result is a quantitative model with connected components labelled consecutively starting from 1, and with the background labelled with 0")>]
