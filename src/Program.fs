...
let rec reduceProgramRec<'a> (env: Environment, prints: List<Eval>) (Program p) (cont: List<Eval> -> 'a) =
    ...
    | command :: commands ->
        ...
        <| fun (env', tasks') -> reduceProgramRec (env', tasks') (Program commands) cont

and reduceCommand (env, prints) command cont =
    match command with
    | Declaration (ide, formalArgs, body) -> cont (env.Bind ide (Fun(env, formalArgs, body)), prints)
    | Print expr -> // TODO: use pos
        reduceExpr env expr
        <| fun result -> cont (env, result :: prints)
...
and reduceExpr (env: Environment) (expr: Expression) (cont: Eval -> 'a) =
    ...
    | ECall (ide, args) -> // TODO: use pos
        ...
        <| fun actualArgs ->
            ...
            | Some (Result r) -> cont r
            | None -> failwith $"unbound identifier {ide}"
