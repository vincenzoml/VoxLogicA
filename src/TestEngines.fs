...
type ArithmeticsResourceKind = KArithmeticsResource
...
type Arithmetics() =
    let opFib =
        OperatorImplementation(
            Requirements([ ("internalAndResult", KArithmeticsResource) ]),
            (fun resources args ->
                let task =
                    new Task<_>(
                        (fun () ->
    ...
    interface ExecutionEngine<ArithmeticsResource, ArithmeticsResourceKind> with
        member __.ImplementationOf s =
            match s with
            | Number n ->
                OperatorImplementation<ArithmeticsResource,ArithmeticsResourceKind>(
                    Requirements([ ("result", KArithmeticsResource) ]),
                    (fun resources _ ->
                        task {
