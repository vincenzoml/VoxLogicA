...
type SITKModel() =
    ...
    interface IBooleanModel<Truth> with
        ...
        member __.BConst v =
            lift (BConst(getBaseTriaGraph().NumSimplexes)) v
    ...
    interface ISpatialModel<Truth> with
        member __.Near v =
            job { return (downClosure (getBaseTriaGraph ()) v) } // NB: this was "upClosure" up to version 0.3 -- Vincenzo 21/02/2024

        member __.Interior v =
            job { return (interior (getBaseTriaGraph ()) v) }

        member __.Through v1 v2 =
            job { return (reach (getBaseTriaGraph ()) v1 v2) }

        member __.CVNear v =
            job { return (upClosure (getBaseTriaGraph ()) v) }
