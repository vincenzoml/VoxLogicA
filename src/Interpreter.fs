...
exception InterpreterException of st: StackTrace * e: exn with
    override this.Message = this.e.Message + (string this.st)

exception UnknownIdentifierException of ide: string with
    override this.Message = sprintf "Unknown identifier %s" this.ide

exception CantSaveException of t: Type * s: string with
    override this.Message =
        sprintf "Saving expression of type %s to file \"%s\" is not supported" (this.t.ToString()) this.s

exception CantPrintException of t: Type with
    override this.Message =
        sprintf "Printing expression of type %s is not supported" (this.t.ToString())


exception MalformedPathException of prefix: string * file: string * result: string * confined: string with
    override this.Message =
        sprintf
            "Malformed path: prefix %s, filename %s, result %s, cannot be confined to %s"
            this.prefix
            this.file
            this.result
            this.confined

exception ImportNotFoundException of fname: string * libdir: string with
    override this.Message =
        sprintf "Import \"%s\" not found\n(local paths are searched in current director and \"%s\")" this.fname
            this.libdir
...
type private DVal =
    | Form of Formula
    | Fun of string list * Expression * Env
...
type Interpreter(model: IModel, checker: ModelChecker) =
    ...
    let rec translateExpression stack
                                (model: #IModel)
                                (formFactory: FormulaFactory)
        ...
        | Call (pos, ide, args) ->
            ...
            if env.ContainsKey ide then
                ...
                | Fun (fargs, body, declenv) ->
                    ...
                    let exenv =
                        try
                            bindList declenv fargs actargs
                        with _ ->
                            raise
                                (InterpreterException
                                    (StackTrace((ide, pos) :: stack),
                                     WrongNumberOfArgumentsException(ide, fargs.Length, actargs.Length)))

                    translateExpression ((ide, pos) :: stack) model formFactory opFactory exenv body
            else
                let args' =
                    Array.ofList (List.map (translateExpression stack model formFactory opFactory env) args)
                ...
                | :? TypeErrorException as e -> raise (InterpreterException(StackTrace((ide, pos) :: stack), e))
                | e -> raise e
    ...
    let interpreterJob libdir filename (model: #IModel) (checker: ModelChecker) (s: System.IO.Stream) =
        let rec evaluate (env: Env) (parsedImports: Set<string>) syn jobs =
            ...
            | ModelSave (pos, fname, expression) :: rest -> // TODO Use interpreterexception also in load and import
                ...
                if model.CanSave typ fname then
                    let j =
                        job {
            ...
            | Import fname :: rest ->
                let find filename =
                    if File.Exists filename then
                        ...
                        if File.Exists nfilename then Some nfilename else None
                ...
                let path =
                    ...
                    | None ->
                        if not (fname.StartsWith "/") then
                            let try2 =
                                System.IO.Path.GetFullPath(System.IO.Path.Combine(libdir, fname))
