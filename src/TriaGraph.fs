...
type IntFileTriaGraph =
    { coordinatesOfPoints: int array list
      atomNames: string list
      simplexes: IntSimplex list }
      ...
      simplexes =
        [ for simplex in 0 .. triaGraph.NumSimplexes - 1 ->
              { id = triaGraph.SimplexId.[simplex]
                points =
                  [ for point in triaGraph.PointsOfSimplex.[simplex] ->
                        Array.findIndex (fun p -> p = point) triaGraph.Points ]
                atoms =
                  let oldAtoms =
                      [ for atom in triaGraph.AtomsOfSimplex.[simplex] -> triaGraph.NameOfAtom.[atom] ]

                  if truth.[simplex] then
                      atomName :: oldAtoms
                  else
                      oldAtoms } ] }
...
let saveTriaGraph triaGraph (filename: string) atom truth =
    ...
    | _ ->
        raise
        <| CantSaveException(TValuation TBool, extension)
...
let private mkTriaGraph (fg: IntFileTriaGraph) =
    ...
    let atomOfName =
        fun s -> Array.findIndex (fun x -> x = s) nameOfAtom
    ...
    List.iteri
        (fun idx simplex ->
            ...
            Array.iteri (fun i c -> coordinatesOfPoint.[idx, i] <- c) fg.coordinatesOfPoints.[idx])
        (List.filter (fun simplex -> (List.length simplex.points) = 1) fg.simplexes)
...
let downClosure triaGraph (truth: Truth) : Truth =
    Array.mapi
        (fun idx s ->
            s
            || Set.exists (Array.get truth) triaGraph.Parents.[idx])
        truth
...
let upClosure triaGraph (truth: Truth) : Truth =
    Array.mapi
        (fun idx s ->
            s
            || Set.exists (Array.get truth) triaGraph.Faces.[idx])
        truth
...
let interior triaGraph (truth: Truth) : Truth =
    Array.mapi
        (fun idx s ->
            s
            && Set.forall (Array.get truth) triaGraph.Faces.[idx])
        truth
...
let openStar triaGraph (truth: Truth) : Truth =
    Array.map2 (||) (downClosure triaGraph truth) (upClosure triaGraph truth)

// Compute the open star of a single simplex as a set
let openStarSimplex triaGraph simplex : Set<int> =
    let others =
        Set.union triaGraph.Parents.[simplex] triaGraph.Faces.[simplex]
...
let reach triaGraph (safe: Truth) (target: Truth) : Truth =
    // TESTING
    let intermediate =
        Array.map2 (&&) (safe) (upClosure triaGraph target) // starting set of the flooding algorithm: simplexes in safe with a face in target
    ...
    let startList =
        [ for i in 0 .. intermediate.Length - 1 do
              if intermediate.[i] then yield i ]
